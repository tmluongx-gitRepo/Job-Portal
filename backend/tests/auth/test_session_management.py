...
class TestSessionManagement:
    ...
    async def test_multiple_concurrent_sessions_allowed(self, client: AsyncClient, test_cleaner):
        ...
        if first_token:
            user_response = await client.get("/api/users/me",
                                            headers={"Authorization": f"Bearer {first_token}"})
            if user_response.status_code == 200:
                test_cleaner.track_user(user_response.json()["id"], email)

        # Login from "device 1"
        response1 = await client.post("/api/auth/login", json={"email": email, "password": password})
        if response1.status_code != 200:
            pytest.skip("Login endpoint not returning tokens")
        token1 = response1.json()["access_token"]

        # Login from "device 2"
        response2 = await client.post("/api/auth/login", json={"email": email, "password": password})
        token2 = response2.json()["access_token"]

        # Login from "device 3"
        response3 = await client.post("/api/auth/login", json={"email": email, "password": password})
